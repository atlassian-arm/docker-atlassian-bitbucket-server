...
def test_import_repository(ctx, tdata):
    clone_o = subprocess.run(
        ["git", "clone", tdata.repo_to_clone, tdata.folder])
    ...
    clone_o = subprocess.run(["git", "remote", "add", tdata.project_key, repo_host_url], cwd=tdata.folder)

    logging.info("Git clone returned with exit code: %d", clone_o.returncode)

    push_o = subprocess.run(["git", "push", "-u", tdata.project_key, "--all"], cwd=tdata.folder)
