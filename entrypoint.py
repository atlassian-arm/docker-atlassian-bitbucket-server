...
if str2bool(env['search_enabled']) is False or env['application_mode'] == 'mirror':
    ...
    logging.warning("######################################################################")
    logging.warning("Starting Bitbucket with embedded search. Note that this is\n" \
                    "not a recommended configuration and is known to have issues with\n" \
                    "clean shutdown. Ideally search should be started in a separate\n" \
                    "container/pod.")
    logging.warning("######################################################################")
